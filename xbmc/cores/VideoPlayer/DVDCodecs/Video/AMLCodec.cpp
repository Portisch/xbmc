...
static int m4s2_dx50_mp4v_write_header(am_private_t *para, am_packet_t *pkt)
{
    ...
    int ret = m4s2_dx50_mp4v_add_header(para->extradata.GetData(), para->extradata.GetSize(), pkt);
    if (ret == PLAYER_SUCCESS) {
        if (1) {
...
bool CAMLCodec::OpenDecoder(CDVDStreamInfo &hints)
{
  ...
  CLog::Log(LOGNOTICE, "CAMLCodec::OpenDecoder - using V4L2 pts format: {}", m_ptsIs64us ? "64Bit":"32Bit");
...
DRAIN:
  if (m_amlVideoFile->IOControl(VIDIOC_DQBUF, &vbuf) < 0)
  {
    ...
    if (!m_drain && elapsed < std::chrono::milliseconds(300))
      CLog::Log(LOGDEBUG, LOGAVTIMING, "CAMLCodec::DequeueBuffer skipped by {}", strerror(errno));
...
CDVDVideoCodec::VCReturn CAMLCodec::GetPicture(VideoPicture *pVideoPicture)
{
  ...
  if (DequeueBuffer() == 0)
  {
    ...
    CLog::Log(LOGDEBUG, LOGVIDEO, "CAMLCodec::GetPicture: index: {:d}, pts: {:.4f}[{:X}], overflow: {:X}",m_bufferIndex, pVideoPicture->pts/DVD_TIME_BASE, m_cur_pts, m_ptsOverflow);
